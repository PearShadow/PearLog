...
class Timesheets extends Repository
{
    ...
    /* getMonthlyTimesheets - get timesheet entries for a full month (for monthly view) */
    public function getMonthlyTimesheets(int $projectId, CarbonInterface $fromDate, int $userId = 0, int $daysInMonth = 31): array
    {
        $query = 'SELECT
            zp_timesheets.id,
            zp_timesheets.userId,
        ...
        $endDate = $fromDate->copy()->addDays($daysInMonth);
        $call->bindValue(':dateEnd', $endDate);
        $call->bindValue(':userId', $userId, PDO::PARAM_INT);
